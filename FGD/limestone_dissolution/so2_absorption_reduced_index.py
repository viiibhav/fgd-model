# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 10:37:38 2019
...
# Equilibrium constants (Radian corp. data, 1970) [mol/L ??]
T = 25 + 273.15  # [K]
R = 8.314        # [J/mol-K]
...
# flag to switch between different boundary conditions
flag = 0
...
def _rc_CaSO4(m, x):
    A_gypsum = 1.35e+04  # [m**2/m**3 slurry]
...
def _sulfite(m, x):
    if flag == 0:
        if x == 0:
            ...
            expr = sum(m.c[k, x]
                       for k in ["SO2", "HSO3-", "SO32-", "CaSO3", "MgSO3"])
            return m.H_SO2 * expr == m.p["SO2"]
#            expr2 = sum(m.J[k, x]
#                        for k in ["SO2", "HSO3-", "SO32-", "CaSO3", "MgSO3"])
#            return kG["SO2"] * (p["SO2"] - m.H_SO2 * expr) == expr2
        else:
            expr = 0
            for k in ["SO2", "HSO3-", "SO32-", "CaSO3", "MgSO3"]:
        ...
        if x == 0 or x == r:
            ...
            expr = 0
            for k in ["SO2", "HSO3-", "SO32-", "CaSO3", "MgSO3"]:
                expr += m.Diff[k] * (m.dudx[k, x] - 2 / (r - x) * m.u[k, x])
            return expr == 0
...
def _carbonate(m, x):
    if x == 0:
        if flag == 0:
            expr = sum((m.c[k, x+dx] - m.c[k, x]) / dx for k in ["CO2",
                       "HCO3-", "CO32-", "CaCO3", "CaHCO3+", "MgHCO3+"])
#            expr = sum((m.c[k, x+dx] - m.c[k, x]) / dx for k in ["CO2",
#                       "HCO3-", "CO32-", "CaCO3"])
            return expr == 0
        else:
        ...
        if flag == 0:
            return sum(m.J[k, x] for k in ["CO2", "HCO3-", "CO32-", "CaCO3",
                       "CaHCO3+", "MgHCO3+"]) == 0
#            return sum(m.J[k, x] for k in
#                       ["CO2", "HCO3-", "CO32-", "CaCO3"]) == 0
        else:
            return Constraint.Skip
    else:
        expr = 0
        for k in ["CO2", "HCO3-", "CO32-", "CaCO3", "CaHCO3+", "MgHCO3+"]:
#        for k in ["CO2", "HCO3-", "CO32-", "CaCO3"]:
            expr += m.Diff[k] * (m.dudx[k, x] - 2 / (x) * m.u[k, x])
        expr += m.rd_CaCO3[x]
        return expr == 0
...
#            return sum(m.J[k, x] for k in ["Mg2+", "MgSO3", "MgSO4"]) == 0
        else:
            return Constraint.Skip
    else:
        expr = 0
...
def _carbondioxide(m, x):
    ...
    if x == 0:
        ...
        if flag == 0:
#            return m.J[k, x] == kG[k] * (m.p[k] - m.p_i[k])
