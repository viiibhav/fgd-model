# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 11:04:55 2019
...
def _CaSO4_diss_der2(m, x):
    if x == 0:
        ...
        return m.Keq["CaSO4"] * m.dudx["CaSO4", x] == \
            m.dudx["Ca2+", x] * m.c["SO42-", x] + \
            m.c["Ca2+", x] * m.dudx["SO42-", x] + \
...
def _MgSO3_diss_der2(m, x):
    if x == 0:
        return m.u["MgSO3", x] == 0
    else:
        return m.Keq["MgSO3"] * m.dudx["MgSO3", x] == \
            m.dudx["Mg2+", x] * m.c["SO32-", x] + \
            m.c["Mg2+", x] * m.dudx["SO32-", x] + \
...
def _MgHCO3_diss_der2(m, x):
    if x == 0:
        return m.u["MgHCO3+", x] == 0
    else:
        return m.Keq["MgHCO3+"] * m.dudx["MgHCO3+", x] == \
               m.dudx["Mg2+", x] * m.c["HCO3-", x] + \
               m.c["Mg2+", x] * m.dudx["HCO3-", x] + \
...
def _MgSO4_diss_der2(m, x):
    if x == 0:
        return m.u["MgSO4", x] == 0
    else:
        return m.Keq["MgSO4"] * m.dudx["MgSO4", x] == \
            m.dudx["Mg2+", x] * m.c["SO42-", x] + \
            m.c["Mg2+", x] * m.dudx["SO42-", x] + \
...
def _reform_con(m, i, x):
    if i in diff_vars:
        if x == r:  # r for symmetry condition at the center
